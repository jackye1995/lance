...
A topic is a Lance table with a user-defined schema. If no schema is
...
The schema must have at least one field marked as a Lance unenforced primary
key. Fields with the `__lance_topic_` prefix are reserved. The topic
automatically appends two system columns to the stored schema:

- `__lance_topic_producer_id: Utf8` — identifies the producer
- `__lance_topic_producer_processing_ts_millis: UInt64` — producer wall-clock timestamp
...
- `send` is best-effort across partitions — some partitions may succeed
  before another fails. It is not transactional.
...
│   ← Topic table (user schema + system columns)
│   └── _mem_wal/
...
### Create a Topic with Default Schema
...
### Create a Topic with Custom Schema

```rust
let schema = ArrowSchema::new(vec![
    Field::new("event_id", DataType::Utf8, false)
        .with_metadata(HashMap::from([
            ("lance-schema:unenforced-primary-key".into(), "true".into()),
        ])),
    Field::new("user_id", DataType::Int64, false),
    Field::new("embedding", DataType::FixedSizeList(
        Arc::new(Field::new("item", DataType::Float32, false)), 1024,
    ), true),
]);
let topic = Topic::builder()
    .directory("/tmp/lance-topics", ["website", "events"])
...
With the default schema, use the `topic_message!` / `topic_messages!` macros:

```rust
let producer = topic.producer("producer-server-1").await?;
producer.send(topic_message!("order-123", json!({ "status": "created" }))?).await?;
producer.send(topic_messages!(
    ["order-124", "order-125"],
    [json!({ "total": 18.00 }), json!({ "total": 73.25 })]
)?).await?;
```

With a custom schema, construct a `RecordBatch` matching the user schema:

```rust
let batch = RecordBatch::try_new(topic.user_schema().clone(), columns)?;
producer.send(batch).await?;
```

`send` acknowledges after the WAL entry is written. If a batch touches
multiple logical partitions, the producer writes one WAL entry per
partition.
